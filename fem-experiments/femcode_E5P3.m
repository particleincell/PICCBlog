...
[p,t]=distmesh2d(fd,@huniform,0.2,[-3,-3;3,3],[-3,-3;3,-3;3,3;-3,3]);
be=boundedges(p,t);
b=unique(be);
Nnorm = size(p,1);
 
f=vectorize(inline('-128*exp(-4*((x-1)^2 + y^2)) - 128*(x-1)*(-8*(x-1))*exp(-4*((x-1)^2 + y^2)) - 128*exp(-4*((x-1)^2 + y^2)) - 128*y*(-4*2*y)*exp(-4*((x-1)^2 + y^2))    -128*exp(-4*((x+1)^2 + y^2)) - 128*(x+1)*(-8*(x+1))*exp(-4*((x+1)^2 + y^2)) - 128*exp(-4*((x+1)^2 + y^2)) - 128*y*(-4*2*y)*exp(-4*((x+1)^2 + y^2))','x','y'));
k=vectorize(inline('1','x','y'));
 
u=vectorize(inline('-16*exp(-4*((x-1)^2 + y^2)) -16*exp(-4*((x+1)^2 + y^2))','x','y'));
ux=vectorize(inline('64*(2*x-2)*exp(-4*((x-1)^2 + y^2)) + 64*(2*x+2)*exp(-4*((x+1)^2 + y^2))','x','y'));
uy=vectorize(inline('64*2*y*exp(-4*((x-1)^2 + y^2)) + 64*2*y*exp(-4*((x+1)^2 + y^2))','x','y'));
 
% for cubic element we need 10 nodes per triangle, so add 2 nodes to each triangle edge midpoint, and 1 at triangle centroid
...
view(2),axis([-3 3 -3 3]),axis equal,colorbar
