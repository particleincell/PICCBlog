...
%norm 6 point quadrature over triangle
  ...
  Ke = w(1,1)*kvals(1)*(gradV(1,:)'*gradV(1,:)) + w(2,1)*kvals(2)*(gradV(2,:)'*gradV(2,:)) + w(3,1)*kvals(3)*(gradV(3,:)'*gradV(3,:)) + w(4,1)*kvals(4)*(gradV(4,:)'*gradV(4,:)) + w(5,1)*kvals(5)*(gradV(5,:)'*gradV(5,:)) + w(6,1)*kvals(6)*(gradV(6,:)'*gradV(6,:)); % element matrix from gradients Vx
  Ke = Ke + w(1,1)*kvals(1)*(gradV(7,:)'*gradV(7,:)) + w(2,1)*kvals(2)*(gradV(8,:)'*gradV(8,:)) + w(3,1)*kvals(3)*(gradV(9,:)'*gradV(9,:)) + w(4,1)*kvals(4)*(gradV(10,:)'*gradV(10,:)) + w(5,1)*kvals(5)*(gradV(11,:)'*gradV(11,:)) + w(6,1)*kvals(6)*(gradV(12,:)'*gradV(12,:)); % element matrix from gradients Vy
