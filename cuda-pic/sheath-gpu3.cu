...
#define NUM_IONS 250000		    // number of ions
#define NUM_ELECTRONS 250000	// number of electrons
...
struct Domain
{
	...
	float *ef;			/*electric field on the cpu*/
...
void ComputeEF(double *phi, float *ef, float *ef_gpu);
void PushSpecies(Species *species, float *ef);
void RewindSpecies(Species *species, float *ef);
void AddParticle(Species *species, double x, double v);	
double XtoL(double pos);
void scatter(double lc, double value, double *field);
double gather(double lc, float *field);
void WriteResults(int ts);

/*gpu versions of above functions to avoid needing compute_35*/
__device__ float dev_XtoL(float pos);
__device__ float dev_gather(float lc, float *field);
__device__ void dev_scatter(float lc, float value, float *field);
...
int main()
{
	...
	domain.ef  = new float[domain.ni];		//electric field
	...
	float *ef_gpu;
	CUDA_ERROR(cudaMalloc((void**)&ef_gpu,domain.ni*sizeof(float)));
	...
	CUDA_ERROR(cudaMemcpy(ions.part_gpu,ions.part,NUM_IONS*sizeof(Particle),cudaMemcpyHostToDevice));
	CUDA_ERROR(cudaMemcpy(electrons.part_gpu,electrons.part,NUM_IONS*sizeof(Particle),cudaMemcpyHostToDevice));
...
__global__ void scatterParticle(Particle *particles, float *den, long N)
{
	...
	{
		float lc = dev_XtoL(particles[p].x);
...
void ComputeEF(double *phi, float *ef, float *ef_gpu)
{
	...
	/*copy to the gpu*/
	CUDA_ERROR(cudaMemcpy(ef_gpu,ef,domain.ni*sizeof(float),cudaMemcpyHostToDevice));
}

/*GPU code to move particles*/
__global__ void moveParticle(Particle *particles, float *ef, float qm, long N)
{
	/*get particle id*/
	long p = blockIdx.x*blockDim.x+threadIdx.x;
	...
	{			
		...
		float lc = dev_XtoL(part->x);
		
		/*gather electric field onto particle position*/
		float part_ef = dev_gather(lc,ef);
...
{
	/*precompute q/m*/
	float qm = species->charge / species->mass;
	
	/*loop over particles*/
	int nblocks = 1+species->np_alloc/THREADS_PER_BLOCK;
	moveParticle<<<nblocks,THREADS_PER_BLOCK>>>(species->part_gpu,ef,qm,species->np_alloc);
}
...
__device__ float dev_gather(float lc, float *field)
{
	...
	/*gather field value onto particle position*/
	float val = field[i]*(1-di) + field[i+1]*(di);
	return val;
}
